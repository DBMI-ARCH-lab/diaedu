...
module Diaedu
  class TriggersController < ::ApplicationController
    ...
    def index
      ...
      render(:json => {
        :objs => Diaedu::Trigger.filter_with(@filter).includes(:tags).offset((page - 1) * PER_PAGE).limit(PER_PAGE).all.as_json(:include => :tags),
        :per_page => PER_PAGE,
        :total_count => Diaedu::Trigger.filter_with(@filter).count
      })
    end
